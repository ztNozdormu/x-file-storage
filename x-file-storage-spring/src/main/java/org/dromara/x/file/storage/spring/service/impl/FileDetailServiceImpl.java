...
public class FileDetailServiceImpl extends ServiceImpl<FileDetailMapper, FileDetail>
        implements FileRecorder, FileDetailService {

    private final ObjectMapper objectMapper = new ObjectMapper();

    private FilePartDetailServiceImpl filePartDetailService;

    public FileDetailServiceImpl(FileDetailMapper fileDetailMapper) {
        super.baseMapper = fileDetailMapper;
    }

    public void setFilePartDetailService(FilePartDetailServiceImpl filePartDetailService) {
    ...
    @Override
    public boolean save(FileInfo info, String tableSuffix) {
        MybatisPlusUtils.setDynamicTableName(BaseTableNameEnum.HOS_FILE_DETAIL.tableName() + "_" + tableSuffix);
        return save(info);
    }

    @Override
    public void update(FileInfo fileInfo, String tableSuffix) {
        MybatisPlusUtils.setDynamicTableName(BaseTableNameEnum.HOS_FILE_DETAIL.tableName() + "_" + tableSuffix);
        update(fileInfo);
    }

    @Override
    public FileInfo getByUrl(String url, String tableSuffix) {
        MybatisPlusUtils.setDynamicTableName(BaseTableNameEnum.HOS_FILE_DETAIL.tableName() + "_" + tableSuffix);
        return getByUrl(url);
    }

    @Override
    public boolean delete(String url, String tableSuffix) {
        MybatisPlusUtils.setDynamicTableName(BaseTableNameEnum.HOS_FILE_DETAIL.tableName() + "_" + tableSuffix);
        return delete(url);
    }
}
